...
fn main() {
    ...
    let pool_path =
        "../../serum/serum_pools/7dLVkUfBVfCGkFhSXDCq1ukM9usathSgS716t643iFGF_serum_dex.json";
    ...
    let ixs = vec![vec![create_account_ix], init_ix, ix, swap_ix].concat();

    let mut tx = Transaction::new_with_payer(&ixs, Some(&owner.pubkey()));

    let base_balance_src = connection
        .get_token_account_balance(&base_ata)
        .unwrap()
        .amount
        .parse::<u64>()
        .unwrap();
    let quote_balance_src = connection
        .get_token_account_balance(&quote_ata)
        .unwrap()
        .amount
        .parse::<u64>()
        .unwrap();
    println!(
        "base, quote balance: {} {}",
        base_balance_src, quote_balance_src
    );
    ...
    let base_balance_dst = connection
        .get_token_account_balance(&base_ata)
        .unwrap()
        .amount
        .parse::<u64>()
        .unwrap();
    let quote_balance_dst = connection
        .get_token_account_balance(&quote_ata)
        .unwrap()
        .amount
        .parse::<u64>()
    ...
    println!(
        "base, quote balance: {} {}",
        base_balance_dst, quote_balance_dst
    );

    if side == "buy" {
        println!(
            "SWAP: quote_diff: -{} base_diff: {}",
            quote_balance_src - quote_balance_dst,
            base_balance_dst - base_balance_src
        );
    } else {
        println!(
            "SWAP: quote_diff: {} base_diff: -{}",
            quote_balance_dst - quote_balance_src,
