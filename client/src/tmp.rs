...
fn main() {
    ...
    let mut first = true ;
        ...
        let init_token_balance = unpack_token_account(&connection.get_account(&init_token_acc).unwrap().data).amount as u128;
        println!(
  ...
  // given update_pks, find the account withing vec<string> and then return the key from the hashmap 
    ...
    let table = read_txn.open_table(TABLE).unwrap();
        ...
        let account0 = connection.get_account(&update_pks[0]).unwrap();
            pool.set_update_accounts(vec![Some(account0),
            Some(account1),
            Some(account)
            ], cluster.clone());
