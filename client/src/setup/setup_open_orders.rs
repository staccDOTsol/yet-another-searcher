...
fn main() {
    ...
    let oo_path = match cluster {
        Cluster::Localnet => "./data/serum_open_orders.json",
        Cluster::Mainnet => {
            "./data/serum_open_orders.json"
        }
    ...
    let connection = RpcClient::new_with_commitment("https://rpc.shyft.to?api_key=jdXnGbRsn0Jvt5t9", CommitmentConfig::processed());
...
&owner.pubkey(),
        ...
        std::fs::write("./data/serum_open_orders.json", json_market_oo).unwrap();
